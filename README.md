...
The router is simulating links with clients over TCP connections with sockets, and therefore we maintain an unordered map that associates each client's IP with a connection socket. We parse in configurations for the NAPT into an array of napt_entry structs (A napt_entry holds a local IP-port pair and associates it with a port on the WAN IP). 

After opening file descriptors for each TCP connection, select() is used to determine which sockets to read. Iterating through the sockets, the program will receive incoming IP packets and route them appropriately to the correct destination. 

First, the router examines the IP header. If the TTL <= 1, the packet is dropped, and the program will move on to the next packet. Otherwise, every incoming packet will decrement the TTL. The checksum is also calculated in its current state to verify the sanity of the contents. 

Once the source and destination IP addresses are extracted, the program examines the nested header, which can be TCP or UDP. TCP and UDP have their checksum values that need to be verified, and a packet with invalid checksum is dropped. TCP and UDP will also contain the source and destination ports, which will be extracted.

The IP addresses and ports may need to be rewritten depending on if the packet crosses the LAN-WAN boundary. Packets that remain in LAN don't need to be rewritten, since clients on the same subnet are visible to each other. Packets being forwarded from LAN to WAN will refer to the NAPT to determine the correct IP-port pair that it should translate its destination to. If an entry for that given source IP-port pair doesn't exist, the NAPT will allocate an unused port on WAN for that LAN source. Packets can also come from outside the subnet but are restricted from crossing the router if the destination port on the packet is not already present in the NAT, and such unrecognized packets are dropped. If the packet destination specifies an existing port, the source of that packet is translated to the WAP IP-port pair that clients inside the subnet will recognize. After all changes to the packet have been made, it's important to redefine the checksum based on the new data stored in the packet. 

Finally, the packet can be forwarded between clients, regardless if they are on LAN or WAN.
